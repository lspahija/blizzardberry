 ...
 */
function generateResponseForQuery(
  query: string,
  businessDomain: BusinessDomain
): string {
  ...
  if (
    ...
    const revenueMetric = businessDomain.metrics.find(
      (m) => m.type === 'currency' || m.label.toLowerCase().includes('revenue')
    );
    if (revenueMetric) {
  ...
  if (
    ...
    const growthMetric = businessDomain.metrics.find(
      (m) => m.type === 'percentage' || m.label.toLowerCase().includes('growth')
    );
    if (growthMetric) {
  ...
  if (
    ...
    const userMetric = businessDomain.metrics.find(
      (m) =>
  ...
  if (
    ...
    if (
      ...
      businessDomain.dashboardData.items.length > 0
    ) {
      const resolvedCount = businessDomain.dashboardData.items.filter(
 ...
 */
function generateSceneAnimations(
  sceneTemplate: any
): GeneratedScene['animations'] {
  ...
  const animations =
    animationMap[animationType as keyof typeof animationMap] ||
    animationMap.fade;

  // Generate element-specific animations based on scene type
  const elementAnimations = generateElementAnimations(
    sceneTemplate.type,
    animationType
  );
      ...
      elements.push(
        {
          ...
          animation: `${animationType}In`,
          delay: baseDelay,
        },
        {
          selector: '.scene-subtitle',
          animation: `${animationType}In`,
          delay: baseDelay + 300,
        },
        {
