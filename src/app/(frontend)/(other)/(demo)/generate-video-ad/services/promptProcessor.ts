import { BusinessDomain, GeneratedVideo, PromptAnalysis, GeneratedScene } from '../types';
 ...
 */
export async function generateVideoFromPrompt(prompt: string): Promise<GeneratedVideo> {
  ...
  const template = getTemplateForBusinessType(analysis.businessType, analysis.primaryFocus);
...
async function analyzePrompt(prompt: string): Promise<PromptAnalysis> {
  ...
  // SaaS indicators
  if (lowerPrompt.includes('saas') || lowerPrompt.includes('software as a service') || 
      lowerPrompt.includes('platform') || lowerPrompt.includes('dashboard') ||
  ...
  // Analytics indicators  
           ...
           lowerPrompt.includes('insights') || lowerPrompt.includes('reporting') ||
           lowerPrompt.includes('visualization') || lowerPrompt.includes('metrics')) {
  ...
  // E-commerce indicators
           ...
           lowerPrompt.includes('product') || lowerPrompt.includes('sales')) {
    businessType = 'ecommerce';
    primaryFocus = 'product';
  }
  
  // Service indicators
  else if (lowerPrompt.includes('service') || lowerPrompt.includes('support') ||
           lowerPrompt.includes('help') || lowerPrompt.includes('customer') ||
           lowerPrompt.includes('chatbot') || lowerPrompt.includes('ai assistant')) {
 ...
 */
function extractBusinessDomain(prompt: string, analysis: PromptAnalysis): BusinessDomain {
  const companyName = analysis.keyEntities.companyName || extractCompanyName(prompt) || 'Your Company';
  const industry = analysis.keyEntities.industry || inferIndustry(prompt) || 'Technology';
  ...
  const dashboardData = analysis.primaryFocus === 'dashboard' || analysis.primaryFocus === 'data' 
...
function extractCompanyName(prompt: string): string | null {
  ...
  const capitalizedWords = firstWords.match(/^([A-Z][a-zA-Z]*(?:\s+[A-Z][a-zA-Z]*)?)/);
  if (capitalizedWords) {
...
function inferIndustry(prompt: string): string {
  ...
  if (lowerPrompt.includes('finance') || lowerPrompt.includes('banking') || lowerPrompt.includes('payment')) {
    return 'Fintech';
  }
  if (lowerPrompt.includes('education') || lowerPrompt.includes('learning') || lowerPrompt.includes('student')) {
    return 'EdTech';
  }
  if (lowerPrompt.includes('real estate') || lowerPrompt.includes('property')) {
    return 'Real Estate';
  }
  if (lowerPrompt.includes('retail') || lowerPrompt.includes('shopping') || lowerPrompt.includes('ecommerce')) {
    return 'E-commerce';
  }
...
function generateQueriesFromPrompt(prompt: string, focus: string): string[] {
  ...
  const questions = prompt.match(/\b(how|what|when|where|why|show me|tell me|display)[^.!?]*[.!?]/gi);
  if (questions) {
    extractedQueries.push(...questions.map(q => q.trim()));
  }
  
  // Combine extracted and template queries
  const templateQueries = focusQueries[focus as keyof typeof focusQueries] || focusQueries.product;
 ...
 * Generate sample metrics based on business type
 */
function generateMetricsFromPrompt(prompt: string, businessType: string): BusinessDomain['metrics'] {
  const lowerPrompt = prompt.toLowerCase();
  ...
  const businessMetrics = {
    saas: [
      { label: 'Monthly Recurring Revenue', value: 247, type: 'currency' as const },
  ...
  let metrics = businessMetrics[businessType as keyof typeof businessMetrics] || businessMetrics.generic;
 ...
 */
function generateDashboardData(prompt: string, businessType: string): BusinessDomain['dashboardData'] {
  const lowerPrompt = prompt.toLowerCase();
  ...
  const dataTemplates = {
    tickets: [
      { id: '#2401', title: 'Login Issue', description: 'User unable to access dashboard', status: 'Open', priority: 'High' },
      { id: '#2402', title: 'Payment Failed', description: 'Credit card processing error', status: 'Resolved', priority: 'Medium' },
      { id: '#2403', title: 'Feature Request', description: 'Request for dark mode', status: 'In Progress', priority: 'Low' }
    ],
    orders: [
      { id: '#ORD-001', title: 'Premium Subscription', description: 'Annual plan upgrade', status: 'Completed', priority: 'High' },
      { id: '#ORD-002', title: 'API Credits', description: 'Additional API usage', status: 'Processing', priority: 'Medium' },
      { id: '#ORD-003', title: 'Enterprise Plan', description: 'Team plan activation', status: 'Completed', priority: 'High' }
    ],
    users: [
      { id: 'USR-101', title: 'Sarah Johnson', description: 'Product Manager at TechCorp', status: 'Active', priority: 'Premium' },
      { id: 'USR-102', title: 'Mike Chen', description: 'Developer at StartupCo', status: 'Active', priority: 'Standard' },
      { id: 'USR-103', title: 'Emma Wilson', description: 'Analyst at DataFirm', status: 'Inactive', priority: 'Free' }
    ],
    tasks: [
      { id: 'TSK-501', title: 'Dashboard Redesign', description: 'Update user interface components', status: 'In Progress', priority: 'High' },
      { id: 'TSK-502', title: 'API Documentation', description: 'Complete REST API docs', status: 'Completed', priority: 'Medium' },
      { id: 'TSK-503', title: 'Security Audit', description: 'Quarterly security review', status: 'Planned', priority: 'High' }
    ],
    items: [
      { id: 'ITM-001', title: 'Data Analysis', description: 'Monthly performance review', status: 'Active', priority: 'High' },
      { id: 'ITM-002', title: 'User Feedback', description: 'Customer satisfaction survey', status: 'Completed', priority: 'Medium' },
      { id: 'ITM-003', title: 'Feature Update', description: 'New functionality release', status: 'Pending', priority: 'High' }
    ]
  };
  
  return {
    title,
    items: dataTemplates[dataType as keyof typeof dataTemplates] || dataTemplates.items
  };
}
...
function extractKeyEntities(prompt: string): PromptAnalysis['keyEntities'] {
  ...
  // Extract features
  const featureKeywords = ['dashboard', 'analytics', 'chat', 'api', 'integration', 'automation', 'ai', 'ml', 'reporting'];
  const features = featureKeywords.filter(keyword => lowerPrompt.includes(keyword));
  
  // Extract metrics keywords
  const metricKeywords = ['revenue', 'growth', 'users', 'conversion', 'performance', 'engagement', 'retention'];
  const metrics = metricKeywords.filter(keyword => lowerPrompt.includes(keyword));
  
  // Extract use cases
  const useCaseKeywords = ['business', 'enterprise', 'startup', 'team', 'individual', 'company'];
  const useCases = useCaseKeywords.filter(keyword => lowerPrompt.includes(keyword));
 ...
 */
function getTemplateIdForType(businessType: string, primaryFocus: string): string {
  const mapping = {
...
function calculateConfidence(prompt: string, businessType: string): number {
  ...
  const keywords = businessKeywords[businessType as keyof typeof businessKeywords] || [];
