-- Chat and Messages tables for storing conversation history
...
CREATE TABLE chats (
    ...
    end_user_config JSONB DEFAULT '{}', -- Store the complete userConfig object for end user identification
...
CREATE INDEX chats_end_user_config_idx ON chats USING GIN (end_user_config); -- GIN index for JSONB queries
...
CREATE TABLE messages (
    ...
    created_at     TIMESTAMP WITH TIME ZONE DEFAULT now(),
    sequence_order INTEGER                  NOT NULL -- Order within the chat session
);
