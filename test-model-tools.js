...
async function testModelTools(model) {
  try {
    ...
    if (response.ok) {
      ...
      if (error.error?.message?.includes('tool use') || error.error?.message?.includes('function')) {
        console.log(`❌ ${model}: Does not support tools - ${error.error.message}`);
      } else {
        console.log(`⚠️ ${model}: Error (may or may not support tools) - ${error.error?.message || 'Unknown error'}`);
      }
      return false;
    }
  } catch (error) {
